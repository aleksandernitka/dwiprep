...
args.add_argument('out', help='Output file name without the png extension')
...
args.add_argument('-c', '--comp', choices=['sub', 'res'], default='sub',\
    help='Comparison Type. What method to use to calculate the difference between images: \
        sub - subtract one from another or res - calculate squared residuals.')
...
vol1_name = args.vol1.split('/')[-1].replace(args.sub, '').split('.')[0]
vol2_name = args.vol2.split('/')[-1].replace(args.sub, '').split('.')[0]
