...
for v in range(img.shape[-1]):
    ...
    fig.savefig(join(a.output, f'tmp_img{1000+v}.png'))
    plt.close(fig)

# Make GIF
print('Compiling GIF...')
images = [Image.open(join(a.output, i)) for i in ls(join(a.output)) if i.endswith('.png') and i.startswith('tmp_img')]
image1 = images[0]
image1.save(join(a.output), format = "GIF", save_all=True, append_images=images[1:], duration=1000, loop=0)

# Clean up
print('Cleaning up...')
for i in ls(join(a.output)):
    if i.endswith('.png') and i.startswith('tmp_img'):
        remove(join(a.output, i))
